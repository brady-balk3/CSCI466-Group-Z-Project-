...
CREATE TABLE Contributes(Position varchar(100), ConID int, FileID int, PRIMARY KEY(ConID, FileID), FOREIGN KEY (ConID) REFERENCES Contributor(ConID), FOREIGN KEY (FileID) REFERENCES KaraokeFile(FileID));
...
CREATE TABLE KaraokeFile(FileID int NOT NULL AUTO_INCREMENT, ArtistID int, TitleID int, ConID int, PRIMARY KEY(FileID), FOREIGN KEY(ArtistID) REFERENCES Artist(ArtistID), FOREIGN KEY(TitleID) REFERENCES Title(TitleID), FOREIGN KEY(ConID) REFERENCES Contributor(ConID));
